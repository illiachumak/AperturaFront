...
const Sidebar = ({category}) => {
  ...
  return (
      <div className='flex flex-col max-w-full max-[840px]:hidden'>
        <div className="bg-color-prim1 mb-8 w-[280px]  rounded-xs text-white">
          {categories.map((category) => {
            return(
            <div key={category.id}>
              <button
                className={`relative rounded-t-xs text-left pl-4 w-full py-4 fz-14 cursor-default ${activeCategory === category.id ? 'bg-active-prim1' : ''}`}
